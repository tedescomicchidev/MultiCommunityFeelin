...
- **Message transport** (see `Infrastructure/IMessageBus.cs`): pluggable transport with in-memory channels for development and Azure Storage queues for durability in production. Messages flow through a lightweight `Microsoft.Agents.AgentMessage<T>` envelope so every hop carries `from`, `to`, and `correlationId` metadata consistent with the Microsoft Agent Framework contract.
...
- **Extensibility:** Swap `SentimentAnalyzer` with an Azure OpenAI or Agent Framework plug-in while keeping the orchestration contract intact. The custom `Microsoft.Agents.AgentMessage<T>` envelope keeps agents loosely coupled while still surfacing framework-friendly metadata for telemetry and tracing.
